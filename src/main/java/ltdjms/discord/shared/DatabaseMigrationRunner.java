 ...
 *
 * <p>This class wraps Flyway configuration and execution, providing:
 *
 * <ul>
 *   <li>Automatic migration on empty databases
 *   <li>Baseline support for existing databases (via baselineOnMigrate)
 *   <li>Clear error reporting when migrations fail
 * </ul>
 *
 * <p>Non-destructive migrations are stored in {@code classpath:db/migration}.
 */
public final class DatabaseMigrationRunner {
   ...
   *
   * <ul>
   *   <li>Create all tables on an empty database
   *   <li>Apply any pending migrations on an existing database
   *   <li>Baseline existing databases that have not been previously tracked by Flyway
   * </ul>
   *
  ...
  public void migrate(DataSource dataSource) {
    ...
    try {
      ...
      if (result.success) {
        LOG.info(
            "Database migration completed successfully. Applied {} migration(s). Current version:"
                + " {}",
  ...
  public String info(DataSource dataSource) {
    ...
    sb.append("Current schema version: ")
        .append(info.current() != null ? info.current().getVersion() : "none")
        .append("\n");
