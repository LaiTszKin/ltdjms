...
/**
 * Provides locale-aware message formatting for dice games. Supports zh-TW localization for Taiwan
 * users while maintaining English for others.
 */
public final class DiceGameMessages {
  ...
  public static String formatDiceGame1Result(
      DiceGameResult result, String currencyIcon, String currencyName, DiscordLocale locale) {
  ...
  public static String formatDiceGame2Result(
      DiceGame2Result result, String currencyIcon, String currencyName, DiscordLocale locale) {
  ...
  public static String formatInsufficientTokens(long required, long current, DiscordLocale locale) {
    if (isZhTw(locale)) {
      return String.format("遊戲代幣不足！\n" + "需要：%,d 代幣\n" + "目前餘額：%,d 代幣", required, current);
    }
   ...
   * @return the formatted error message
   */
  public static String formatTokenRangeError(
      long playerInput, long minTokens, long maxTokens, DiscordLocale locale) {
    if (isZhTw(locale)) {
      return String.format(
          "代幣投入數量超出範圍！\n" + "您輸入的數量：%,d\n" + "允許範圍：%,d ~ %,d 代幣",
  ...
  /**
   * Formats a "missing tokens input" error message based on the locale. Used when the player does
   * not provide the required tokens parameter.
   *
   * @param minTokens the minimum allowed tokens
   * @param maxTokens the maximum allowed tokens
   * @param locale the user's locale
   * @return the formatted error message
   */
  public static String formatMissingTokensError(
      long minTokens, long maxTokens, DiscordLocale locale) {
    if (isZhTw(locale)) {
      return String.format("請輸入本局要投入的遊戲代幣數量！\n" + "必須介於 %,d ~ %,d 代幣之間", minTokens, maxTokens);
    }
    return String.format(
  ...
  private static String formatDiceGame1ResultZhTw(
      DiceGameResult result, String currencyIcon, String currencyName) {
  ...
  private static String formatDiceGame2ResultZhTw(
      DiceGame2Result result, String currencyIcon, String currencyName) {
