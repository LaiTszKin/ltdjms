 ...
 *
 * <p>此類別將 JDA 的 {@link GenericInteractionCreateEvent} 包裝為統一的 {@link DiscordInteraction} 介面，提供與 JDA
 * 實作細節無關的抽象層。
 *
 * <p>主要功能：
 *
 * <ul>
 *   <li>從 JDA 事件中提取 Guild ID 和 User ID
 *   <li>委派所有互動操作到 InteractionHook
 *   <li>追蹤互動狀態（acknowledged）
 * </ul>
 */
public class JdaDiscordInteraction implements DiscordInteraction {
   ...
   *
   * <p>不同類型的事件可能有不同的方式取得 Hook。 對於 SlashCommandInteractionEvent 和按鈕事件，Hook 在事件確認後可用。
   *
   * @param event JDA 互動事件
  ...
  private InteractionHook extractHook(GenericInteractionCreateEvent event) {
    ...
    try {
      ...
      // 暫時的解決方案：使用事件類型特定的方法
      if (event
          instanceof net.dv8tion.jda.api.events.interaction.command.SlashCommandInteractionEvent) {
      ...
      if (event
          instanceof net.dv8tion.jda.api.events.interaction.component.ButtonInteractionEvent) {
