...
/**
 * ProductRedemptionTransactionRepository 的 JDBC 實作。
 * 提供儲存和查詢商品兌換交易歷史的方法。
 */
public class JdbcProductRedemptionTransactionRepository implements ProductRedemptionTransactionRepository {

    private static final Logger LOG = LoggerFactory.getLogger(JdbcProductRedemptionTransactionRepository.class);
    ...
    public ProductRedemptionTransaction save(ProductRedemptionTransaction transaction) {
        String sql = "INSERT INTO product_redemption_transaction " +
                "(guild_id, user_id, product_id, product_name, redemption_code, quantity, reward_type, reward_amount, created_at) " +
                "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?) RETURNING id";
        ...
        try (Connection conn = dataSource.getConnection();
            ...
            setNullableString(stmt, 7, transaction.rewardType() != null ? transaction.rewardType().name() : null);
            ...
            try (ResultSet rs = stmt.executeQuery()) {
                if (rs.next()) {
                    ...
                    ProductRedemptionTransaction saved = new ProductRedemptionTransaction(
                            id,
                            transaction.guildId(),
                            transaction.userId(),
                    ...
                    LOG.debug("Saved product redemption transaction: id={}, guildId={}, userId={}, product={}",
                            id, transaction.guildId(), transaction.userId(), transaction.productName());
                    return saved;
                } else {
            ...
            LOG.error("Failed to save product redemption transaction for guildId={}, userId={}",
                    transaction.guildId(), transaction.userId(), e);
            throw new RepositoryException("Failed to save product redemption transaction", e);
        }
    }

    @Override
    public List<ProductRedemptionTransaction> findByGuildIdAndUserId(long guildId, long userId, int limit, int offset) {
        String sql = "SELECT id, guild_id, user_id, product_id, product_name, redemption_code, " +
        ...
        try (Connection conn = dataSource.getConnection();
            ...
            LOG.debug("Found {} product redemption transactions for guildId={}, userId={} (limit={}, offset={})",
                    transactions.size(), guildId, userId, limit, offset);
            return transactions;

        } catch (SQLException e) {
            LOG.error("Failed to find product redemption transactions for guildId={}, userId={}", guildId, userId, e);
    ...
    public long countByGuildIdAndUserId(long guildId, long userId) {
        String sql = "SELECT COUNT(*) FROM product_redemption_transaction WHERE guild_id = ? AND user_id = ?";
        ...
        try (Connection conn = dataSource.getConnection();
            ...
            LOG.error("Failed to count product redemption transactions for guildId={}, userId={}", guildId, userId, e);
    ...
    public int deleteByGuildIdAndUserId(long guildId, long userId) {
        ...
        try (Connection conn = dataSource.getConnection();
            ...
            if (affected > 0) {
                LOG.info("Deleted {} product redemption transactions for guildId={}, userId={}", affected, guildId, userId);
            }
            return affected;

        } catch (SQLException e) {
            LOG.error("Failed to delete product redemption transactions for guildId={}, userId={}", guildId, userId, e);
