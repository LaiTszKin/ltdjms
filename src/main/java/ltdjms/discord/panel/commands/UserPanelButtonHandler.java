...
/**
 * Handles button interactions for the user panel. Processes token history viewing, pagination,
 * redemption code input, and navigation back to main panel.
 */
public class UserPanelButtonHandler extends ListenerAdapter {
  ...
  public static final String BUTTON_PREFIX_PRODUCT_REDEMPTION_HISTORY =
      "user_panel_product_redemption_history";
  public static final String BUTTON_PREFIX_PRODUCT_REDEMPTION_PAGE =
      "user_panel_product_redemption_page_";
  ...
  public void onButtonInteraction(ButtonInteractionEvent event) {
    ...
    LOG.debug(
        "Processing button interaction: buttonId={}, guildId={}, userId={}",
        buttonId,
        guildId,
        userId);
  ...
  private void openRedemptionModal(ButtonInteractionEvent event) {
    TextInput codeInput =
        TextInput.create("code", "兌換碼", TextInputStyle.SHORT)
    ...
    Modal modal = Modal.create(MODAL_REDEEM, "兌換碼").addComponents(ActionRow.of(codeInput)).build();
  ...
  private void showMainPanel(ButtonInteractionEvent event, long guildId, long userId) {
    ...
    if (result.isErr()) {
      LOG.error(
          "Failed to load user panel view: guildId={}, userId={}, error={}",
          guildId,
          userId,
  ...
  private void showTokenHistoryPage(
      ButtonInteractionEvent event, long guildId, long userId, int page) {
    TransactionPage transactionPage =
        userPanelService.getTokenTransactionPage(guildId, userId, page);
    ...
    LOG.debug("Showed token history page {} for guildId={}, userId={}", page, guildId, userId);
  }

  private void showCurrencyHistoryPage(
      ButtonInteractionEvent event, long guildId, long userId, int page) {
    ...
    LOG.debug("Showed currency history page {} for guildId={}, userId={}", page, guildId, userId);
  }

  private MessageEmbed buildTokenHistoryEmbed(TransactionPage page) {
    EmbedBuilder builder = new EmbedBuilder().setTitle("📜 遊戲代幣流水").setColor(EMBED_COLOR);
    ...
    if (page.isEmpty()) {
      ...
      for (GameTokenTransaction tx : page.transactions()) {
        sb.append(tx.getShortTimestamp()).append(" ").append(tx.formatForDisplay()).append("\n");
      }
  ...
  private MessageEmbed buildCurrencyHistoryEmbed(CurrencyTransactionService.TransactionPage page) {
    EmbedBuilder builder = new EmbedBuilder().setTitle("💰 貨幣流水").setColor(EMBED_COLOR);
  ...
  private List<Button> buildTokenPaginationButtons(TransactionPage page) {
    ...
    if (page.hasPreviousPage()) {
      buttons.add(Button.secondary(BUTTON_PREFIX_TOKEN_PAGE + (page.currentPage() - 1), "⬅️ 上一頁"));
    }

    if (page.hasNextPage()) {
      buttons.add(Button.secondary(BUTTON_PREFIX_TOKEN_PAGE + (page.currentPage() + 1), "下一頁 ➡️"));
    }
  ...
  private List<Button> buildCurrencyPaginationButtons(
    ...
    if (page.hasPreviousPage()) {
      buttons.add(
          Button.secondary(BUTTON_PREFIX_CURRENCY_PAGE + (page.currentPage() - 1), "⬅️ 上一頁"));
    }

    if (page.hasNextPage()) {
      buttons.add(
          Button.secondary(BUTTON_PREFIX_CURRENCY_PAGE + (page.currentPage() + 1), "下一頁 ➡️"));
    }
  ...
  private void showProductRedemptionHistoryPage(
    ...
    LOG.debug(
        "Showed product redemption history page {} for guildId={}, userId={}",
        page,
        guildId,
        userId);
  }
  ...
  private MessageEmbed buildProductRedemptionHistoryEmbed(
    ...
    EmbedBuilder builder = new EmbedBuilder().setTitle("🛒 商品流水").setColor(EMBED_COLOR);
  ...
  private List<Button> buildProductRedemptionPaginationButtons(
    ...
    if (page.hasPreviousPage()) {
      buttons.add(
          Button.secondary(
              BUTTON_PREFIX_PRODUCT_REDEMPTION_PAGE + (page.currentPage() - 1), "⬅️ 上一頁"));
    }

    if (page.hasNextPage()) {
      buttons.add(
          Button.secondary(
              BUTTON_PREFIX_PRODUCT_REDEMPTION_PAGE + (page.currentPage() + 1), "下一頁 ➡️"));
    }
