...
/**
 * Listener for domain events that triggers real-time updates for active admin panels. Handles
 * currency configuration, game configuration, and product changes.
 */
public class AdminPanelUpdateListener implements Consumer<DomainEvent> {
  ...
  private void handleCurrencyConfigChanged(CurrencyConfigChangedEvent event) {
    LOG.debug(
        "Admin panel update triggered by currency config change for guildId={}", event.guildId());
  ...
  private void handleDiceGameConfigChanged(DiceGameConfigChangedEvent event) {
    LOG.debug(
        "Admin panel update triggered by {} config change for guildId={}",
        event.gameType(),
        event.guildId());
  ...
  private void handleProductChanged(ProductChangedEvent event) {
    LOG.debug(
        "Admin panel update triggered by product {} for guildId={}, productId={}",
        event.operationType(),
        event.guildId(),
        event.productId());
  ...
  private void handleRedemptionCodesGenerated(RedemptionCodesGeneratedEvent event) {
    LOG.debug(
        "Admin panel update triggered by redemption codes generated for guildId={}, productId={},"
            + " count={}",
        event.guildId(),
        event.productId(),
        event.count());
  ...
  /**
   * Notifies all admin panels in a guild that a change has occurred. Since admin panels have
   * complex state, we log the notification and let the panels refresh on next interaction.
   */
  private void notifyAdminPanels(long guildId, String changeDescription) {
    sessionManager.updatePanelsByGuild(
        ...
        hook -> {
          ...
          // Admin panels are stateful with multiple views (main menu, game config, product list,
          // etc.)
          // A full rebuild requires knowing the current view state, which isn't stored in the
          // session.
