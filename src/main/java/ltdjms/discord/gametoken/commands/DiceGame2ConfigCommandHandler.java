 ...
 * Allows administrators to configure tokens per play range and reward multipliers
 * for dice-game-2.
 */
public class DiceGame2ConfigCommandHandler implements SlashCommandListener.CommandHandler {
    ...
    public void handle(SlashCommandInteractionEvent event) {
        ...
        if (minTokensOption == null && maxTokensOption == null
                && straightMultiplierOption == null && baseMultiplierOption == null
                && tripleLowBonusOption == null && tripleHighBonusOption == null) {
        ...
        try {
            ...
            if (straightMultiplierOption != null || baseMultiplierOption != null) {
                long straightMultiplier = straightMultiplierOption != null
                        ? straightMultiplierOption.getAsLong()
                        : updatedConfig.straightMultiplier();
                long baseMultiplier = baseMultiplierOption != null
                        ? baseMultiplierOption.getAsLong()
                        : updatedConfig.baseMultiplier();

                updatedConfig = configRepository.updateMultipliers(guildId, straightMultiplier, baseMultiplier);
            }

            // Update triple bonuses if any bonus option is provided
            if (tripleLowBonusOption != null || tripleHighBonusOption != null) {
                long tripleLowBonus = tripleLowBonusOption != null
                        ? tripleLowBonusOption.getAsLong()
                        : updatedConfig.tripleLowBonus();
                long tripleHighBonus = tripleHighBonusOption != null
                        ? tripleHighBonusOption.getAsLong()
                        : updatedConfig.tripleHighBonus();

                updatedConfig = configRepository.updateTripleBonuses(guildId, tripleLowBonus, tripleHighBonus);
            }
            ...
            BotErrorHandler.logSuccess(event,
                    "minTokens=" + updatedConfig.minTokensPerPlay() +
                            ", maxTokens=" + updatedConfig.maxTokensPerPlay() +
                            ", straightMultiplier=" + updatedConfig.straightMultiplier() +
                            ", baseMultiplier=" + updatedConfig.baseMultiplier() +
                            ", tripleLowBonus=" + updatedConfig.tripleLowBonus() +
                            ", tripleHighBonus=" + updatedConfig.tripleHighBonus());
