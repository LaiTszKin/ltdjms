...
/**
 * Handler for the /dice-game-1-config slash command. Allows administrators to configure the token
 * range and reward for playing dice-game-1.
 */
  ...
  public void handle(SlashCommandInteractionEvent event) {
    ...
    try {
      ...
      if (minTokensOption != null || maxTokensOption != null) {
        long minTokens =
            minTokensOption != null ? minTokensOption.getAsLong() : current.minTokensPerPlay();
        long maxTokens =
            maxTokensOption != null ? maxTokensOption.getAsLong() : current.maxTokensPerPlay();
      ...
      String message =
          String.format(
              "**骰子遊戲 1 設定已更新！**\n" + "代幣投入範圍：%,d - %,d\n" + "單骰獎勵倍率：%,d",
              current.minTokensPerPlay(), current.maxTokensPerPlay(), current.rewardPerDiceValue());
    ...
    try {
      ...
      String message =
          String.format(
              ...
              config.minTokensPerPlay(), config.maxTokensPerPlay(), config.rewardPerDiceValue());
      event.reply(message).queue();
