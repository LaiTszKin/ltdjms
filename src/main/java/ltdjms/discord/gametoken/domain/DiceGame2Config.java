 ...
 * the valid range of tokens per play, and the reward multipliers/bonuses.
 *
 * <p>Players must explicitly specify how many tokens to spend when playing;
 * there is no default token amount.</p>
 */
public record DiceGame2Config(
    ...
    public DiceGame2Config {
        if (minTokensPerPlay < 0) {
            throw new IllegalArgumentException("minTokensPerPlay cannot be negative: " + minTokensPerPlay);
        }
        if (maxTokensPerPlay < 0) {
            throw new IllegalArgumentException("maxTokensPerPlay cannot be negative: " + maxTokensPerPlay);
        }
        if (minTokensPerPlay > maxTokensPerPlay) {
            throw new IllegalArgumentException("minTokensPerPlay cannot be greater than maxTokensPerPlay: " +
                    minTokensPerPlay + " > " + maxTokensPerPlay);
        }
    ...
    /**
     * Calculates the number of dice to roll based on token amount.
     * Each token produces DICE_PER_TOKEN dice.
     *
