...
/**
 * Service for managing guild currency configuration. Handles creating and updating currency name
 * and icon per guild.
 */
public class CurrencyConfigService {
  ...
  /**
   * Pattern to detect Discord custom emoji format. Matches: {@code <:name:id>} or {@code
   * <a:name:id>} This is a loose pattern to catch anything that looks like a custom emoji, the
   * actual validation is done by EmojiValidator.
   */
  private static final Pattern CUSTOM_EMOJI_PATTERN = Pattern.compile("^<a?:[^:]+:[^>]+>$");
  ...
  /**
   * Gets the currency configuration for a guild. Returns defaults if no custom configuration
   * exists.
   *
  ...
  /**
   * Gets the currency configuration for a guild using Result-based error handling. Returns defaults
  ...
  public Result<GuildCurrencyConfig, DomainError> tryGetConfig(long guildId) {
    try {
      ...
      return Result.err(
          DomainError.persistenceFailure("Failed to retrieve currency configuration", e));
    }
  }

  /**
   * Updates the currency configuration for a guild. Creates a new configuration if one doesn't
   * exist.
   *
   * @param guildId the Discord guild ID
  ...
  public GuildCurrencyConfig updateConfig(long guildId, String name, String icon) {
    ...
    LOG.info(
        "Updated currency config: guildId={}, name={}, icon={}",
        guildId,
        saved.currencyName(),
        saved.currencyIcon());
  ...
  /**
   * Updates the currency configuration for a guild using Result-based error handling. Creates a new
   ...
   */
  public Result<GuildCurrencyConfig, DomainError> tryUpdateConfig(
      long guildId, String name, String icon) {
    LOG.debug("Updating currency config for guildId={}, name={}, icon={}", guildId, name, icon);
    ...
    try {
      ...
      eventPublisher.publish(
          new CurrencyConfigChangedEvent(guildId, saved.currencyName(), saved.currencyIcon()));

      return Result.ok(saved);
    } catch (RepositoryException e) {
      LOG.error("Failed to update currency config for guildId={}", guildId, e);
      return Result.err(
          DomainError.persistenceFailure("Failed to update currency configuration", e));
    }
  }
    ...
    if (name.length() > GuildCurrencyConfig.MAX_NAME_LENGTH) {
      return Result.err(
          DomainError.invalidInput(
              "Currency name cannot exceed "
                  + GuildCurrencyConfig.MAX_NAME_LENGTH
                  + " characters"));
    }
  ...
  private Result<Unit, DomainError> tryValidateIcon(String icon) {
    ...
    if (icon.length() > GuildCurrencyConfig.MAX_ICON_LENGTH) {
      return Result.err(
          DomainError.invalidInput(
              "Currency icon cannot exceed "
                  + GuildCurrencyConfig.MAX_ICON_LENGTH
                  + " characters"));
    }
  ...
  /**
   * Checks if the icon string looks like a Discord custom emoji format. This is a quick check to
   * determine if we need to validate with JDA.
   *
