...
/**
 * Service for adjusting member currency balances. Handles atomic balance changes with validation
...
public class BalanceAdjustmentService {
  ...
  public BalanceAdjustmentResult adjustBalance(long guildId, long userId, long amount) {
    ...
    if (!MemberCurrencyAccount.isValidAdjustmentAmount(amount)) {
      throw new IllegalArgumentException(
          "Amount exceeds maximum: |"
              + amount
              + "| > "
              + MemberCurrencyAccount.MAX_ADJUSTMENT_AMOUNT);
    }
    ...
    LOG.info(
        "Balance adjusted: guildId={}, userId={}, previous={}, new={}, adjustment={}",
        guildId,
        userId,
        previousBalance,
        updated.balance(),
   ...
   * @return Result containing BalanceAdjustmentResult on success, or DomainError on failure
   */
  public Result<BalanceAdjustmentResult, DomainError> tryAdjustBalanceTo(
      long guildId, long userId, long targetBalance) {
    LOG.debug(
        "Adjusting balance to target for guildId={}, userId={}, targetBalance={}",
        guildId,
        userId,
        targetBalance);
    ...
    try {
      ...
      return Result.err(
          DomainError.invalidInput(
              "Adjustment would cause overflow: target="
                  + targetBalance
                  + ", current="
                  + previousBalance));
    }
    ...
    LOG.info(
        "Balance adjusted to target: guildId={}, userId={}, previous={}, new={}, adjustment={}",
        guildId,
        userId,
        previousBalance,
        updated.balance(),
  ...
  private void updateCachedBalance(long guildId, long userId, long balance) {
    ...
    LOG.debug(
        "Updated cached balance: guildId={}, userId={}, balance={}", guildId, userId, balance);
  }
}
