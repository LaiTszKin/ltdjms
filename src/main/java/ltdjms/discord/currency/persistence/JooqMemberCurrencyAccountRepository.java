...
/**
 * JOOQ-based implementation of MemberCurrencyAccountRepository. Provides methods to load, create,
...
public class JooqMemberCurrencyAccountRepository implements MemberCurrencyAccountRepository {
  ...
  private static final Logger LOG =
      LoggerFactory.getLogger(JooqMemberCurrencyAccountRepository.class);
  ...
  public Optional<MemberCurrencyAccount> findByGuildIdAndUserId(long guildId, long userId) {
    try {
      Record record =
          dsl.select(GUILD_ID, USER_ID, BALANCE, CREATED_AT, UPDATED_AT)
  ...
  public MemberCurrencyAccount save(MemberCurrencyAccount account) {
    try {
      int affected =
          dsl.insertInto(MEMBER_CURRENCY_ACCOUNT)
  ...
  public MemberCurrencyAccount adjustBalance(long guildId, long userId, long amount) {
    ...
    try {
      ...
      Record result =
          dsl.update(MEMBER_CURRENCY_ACCOUNT)
              .set(BALANCE, BALANCE.plus(amount))
              .set(UPDATED_AT, now)
              .where(GUILD_ID.eq(guildId).and(USER_ID.eq(userId)).and(BALANCE.plus(amount).ge(0L)))
  ...
  public boolean deleteByGuildIdAndUserId(long guildId, long userId) {
    try {
      int affected =
          dsl.deleteFrom(MEMBER_CURRENCY_ACCOUNT)
              .where(GUILD_ID.eq(guildId).and(USER_ID.eq(userId)))
              .execute();
