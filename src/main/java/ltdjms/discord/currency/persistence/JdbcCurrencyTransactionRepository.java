...
/**
 * JDBC-based implementation of CurrencyTransactionRepository.
 * Provides methods to save and query currency transaction history.
 */
public class JdbcCurrencyTransactionRepository implements CurrencyTransactionRepository {

    private static final Logger LOG = LoggerFactory.getLogger(JdbcCurrencyTransactionRepository.class);
    ...
    public CurrencyTransaction save(CurrencyTransaction transaction) {
        String sql = "INSERT INTO currency_transaction " +
        ...
        try (Connection conn = dataSource.getConnection();
            ...
            try (ResultSet rs = stmt.executeQuery()) {
                if (rs.next()) {
                    ...
                    CurrencyTransaction saved = new CurrencyTransaction(
                            id,
                            transaction.guildId(),
                            transaction.userId(),
                            transaction.amount(),
                    ...
                    LOG.debug("Saved currency transaction: id={}, guildId={}, userId={}, amount={}, source={}",
                            id, transaction.guildId(), transaction.userId(), transaction.amount(), transaction.source());
                    return saved;
                } else {
            ...
            LOG.error("Failed to save currency transaction for guildId={}, userId={}",
    ...
    @Override
    public List<CurrencyTransaction> findByGuildIdAndUserId(long guildId, long userId, int limit, int offset) {
        String sql = "SELECT id, guild_id, user_id, amount, balance_after, source, description, created_at " +
        ...
        try (Connection conn = dataSource.getConnection();
            ...
            LOG.debug("Found {} currency transactions for guildId={}, userId={} (limit={}, offset={})",
                    transactions.size(), guildId, userId, limit, offset);
            return transactions;

        } catch (SQLException e) {
            LOG.error("Failed to find currency transactions for guildId={}, userId={}", guildId, userId, e);
    ...
    public long countByGuildIdAndUserId(long guildId, long userId) {
        ...
        try (Connection conn = dataSource.getConnection();
            ...
            LOG.error("Failed to count currency transactions for guildId={}, userId={}", guildId, userId, e);
    ...
    public int deleteByGuildIdAndUserId(long guildId, long userId) {
        ...
        try (Connection conn = dataSource.getConnection();
            ...
            if (affected > 0) {
                LOG.info("Deleted {} currency transactions for guildId={}, userId={}", affected, guildId, userId);
            }
            return affected;

        } catch (SQLException e) {
            LOG.error("Failed to delete currency transactions for guildId={}, userId={}", guildId, userId, e);
