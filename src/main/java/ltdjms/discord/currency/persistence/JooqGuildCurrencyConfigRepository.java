...
/**
 * JOOQ-based implementation of GuildCurrencyConfigRepository. Provides CRUD operations scoped by
 * guild ID.
 */
public class JooqGuildCurrencyConfigRepository implements GuildCurrencyConfigRepository {

  private static final Logger LOG =
      LoggerFactory.getLogger(JooqGuildCurrencyConfigRepository.class);
  ...
  public Optional<GuildCurrencyConfig> findByGuildId(long guildId) {
    try {
      Record record =
          dsl.select(GUILD_ID, CURRENCY_NAME, CURRENCY_ICON, CREATED_AT, UPDATED_AT)
  ...
  public GuildCurrencyConfig update(GuildCurrencyConfig config) {
    try {
      int affected =
          dsl.update(GUILD_CURRENCY_CONFIG)
              .set(CURRENCY_NAME, config.currencyName())
              .set(CURRENCY_ICON, config.currencyIcon())
              .set(UPDATED_AT, toOffsetDateTime(config.updatedAt()))
  ...
  public boolean deleteByGuildId(long guildId) {
    try {
      int affected = dsl.deleteFrom(GUILD_CURRENCY_CONFIG).where(GUILD_ID.eq(guildId)).execute();
