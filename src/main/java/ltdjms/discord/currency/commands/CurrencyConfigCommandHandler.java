...
/**
 * Handler for the /currency-config slash command. Allows administrators to configure the guild's
...
public class CurrencyConfigCommandHandler implements SlashCommandListener.CommandHandler {
  ...
  public void handle(SlashCommandInteractionEvent event) {
    ...
    Result<GuildCurrencyConfig, DomainError> result =
        configService.tryUpdateConfig(guildId, name, icon);
    ...
    String message =
        String.format(
            ...
            updated.currencyName(), updated.currencyIcon());
    event.reply(message).queue();

    BotErrorHandler.logSuccess(
        event, "name=" + updated.currencyName() + ", icon=" + updated.currencyIcon());
  }

  /**
   * Extracts the icon value from the option mapping. If the option contains a rendered custom emoji
   * (from Discord's autocomplete or paste), extracts the formatted string (e.g., {@code
   * <:name:id>}) from Mentions. Otherwise, returns the raw string value.
   *
  ...
  private void showCurrentConfig(SlashCommandInteractionEvent event, long guildId) {
    ...
    String message =
        String.format(
            ...
            config.currencyName(), config.currencyIcon());
