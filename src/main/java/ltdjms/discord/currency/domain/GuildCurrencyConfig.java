...
/**
 * Represents the currency configuration for a specific Discord guild. Each guild can have exactly
 * one currency configuration with a custom name and icon.
 */
public record GuildCurrencyConfig(
    long guildId, String currencyName, String currencyIcon, Instant createdAt, Instant updatedAt) {
  ...
  /**
   * Maximum length for the currency icon/label (supports multi-codepoint emoji, Discord custom
   * emoji markup, and short text).
   */
  ...
  public GuildCurrencyConfig {
    ...
    if (currencyName.length() > MAX_NAME_LENGTH) {
      throw new IllegalArgumentException(
          "currencyName must not exceed " + MAX_NAME_LENGTH + " characters");
    }
    ...
    if (currencyIcon.length() > MAX_ICON_LENGTH) {
      throw new IllegalArgumentException(
          "currencyIcon must not exceed " + MAX_ICON_LENGTH + " characters");
    }
  }
