...
/**
 * Unit tests for CurrencyConfigService. Tests configuration retrieval, update, and validation
 * logic.
 */
@ExtendWith(MockitoExtension.class)
@SuppressWarnings(
    "deprecation") // validates both deprecated getConfig/updateConfig and new Result-based APIs
class CurrencyConfigServiceTest {
  ...
  @Mock private EmojiValidator emojiValidator;
  ...
  void shouldReturnExistingConfigWhenExists() {
    ...
    GuildCurrencyConfig existingConfig =
  ...
  class ResultBasedApiTests {
    ...
    void tryUpdateConfigShouldReturnPersistenceFailureOnRepositoryError() {
      ...
      when(configRepository.findByGuildId(TEST_GUILD_ID)).thenReturn(Optional.of(existingConfig));
      when(configRepository.saveOrUpdate(any())).thenThrow(new RepositoryException("DB error"));
