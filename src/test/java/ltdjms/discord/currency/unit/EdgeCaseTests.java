...
/**
 * Edge case unit tests for the currency system. Tests invalid amounts, missing configuration,
 * member rejoin scenarios, and permission errors.
 */
@ExtendWith(MockitoExtension.class)
@SuppressWarnings(
    "deprecation") // covers legacy exception-based APIs (getBalance/adjustBalance/updateConfig)
class EdgeCaseTests {
  ...
  class InvalidAmountTests {
    ...
    void shouldAcceptAdjustmentAtExactlyMaximum() {
      ...
      MemberCurrencyAccount adjusted =
          new MemberCurrencyAccount(TEST_GUILD_ID, TEST_USER_ID, maxAmount, now, now);

      when(accountRepository.findOrCreate(TEST_GUILD_ID, TEST_USER_ID)).thenReturn(initial);
      when(accountRepository.adjustBalance(TEST_GUILD_ID, TEST_USER_ID, maxAmount))
          .thenReturn(adjusted);
  ...
  class OverflowProtectionTests {
    ...
    void shouldRejectAdjustmentCausingPositiveOverflow() {
      ...
      MemberCurrencyAccount account =
          new MemberCurrencyAccount(TEST_GUILD_ID, TEST_USER_ID, Long.MAX_VALUE, now, now);
    ...
    void shouldAcceptAdjustmentCloseToOverflowBoundary() {
      Instant now = Instant.now();
      MemberCurrencyAccount account =
          new MemberCurrencyAccount(TEST_GUILD_ID, TEST_USER_ID, Long.MAX_VALUE - 100, now, now);
  ...
  class MissingConfigurationTests {
    ...
    void shouldCreateAccountAutomaticallyWhenBalanceQueriedForFirstTime() {
      Instant now = Instant.now();
      MemberCurrencyAccount newAccount =
          new MemberCurrencyAccount(TEST_GUILD_ID, TEST_USER_ID, 0L, now, now);
  ...
  class MemberLeaveRejoinTests {
    ...
    void shouldAllowAdjustmentForMemberWhoLeftAndRejoined() {
      ...
      MemberCurrencyAccount adjustedAccount =
          new MemberCurrencyAccount(TEST_GUILD_ID, TEST_USER_ID, 300L, createdAt, updatedAt);
  ...
  class CurrencyConfigValidationTests {
    ...
    void shouldAcceptCurrencyNameAtExactlyMaxLength() {
      ...
      GuildCurrencyConfig updatedConfig =
          new GuildCurrencyConfig(TEST_GUILD_ID, exactLengthName, "💰", now, now);
  ...
  class NegativeBalancePreventionTests {
    ...
    void shouldRejectCreatingAccountWithNegativeBalance() {
      ...
      assertThatThrownBy(
              () -> new MemberCurrencyAccount(TEST_GUILD_ID, TEST_USER_ID, -1L, now, now))
          .isInstanceOf(IllegalArgumentException.class)
  ...
  class GuildIsolationTests {
    ...
    void shouldMaintainSeparateBalancesForSameUserInDifferentGuilds() {
      ...
      MemberCurrencyAccount guild1Account =
          new MemberCurrencyAccount(guild1, TEST_USER_ID, 100L, now, now);
      MemberCurrencyAccount guild2Account =
          new MemberCurrencyAccount(guild2, TEST_USER_ID, 500L, now, now);
    ...
    void shouldUseGuildSpecificCurrencyConfiguration() {
      ...
      MemberCurrencyAccount account1 =
          new MemberCurrencyAccount(guild1, TEST_USER_ID, 0L, now, now);
      MemberCurrencyAccount account2 =
          new MemberCurrencyAccount(guild2, TEST_USER_ID, 0L, now, now);
