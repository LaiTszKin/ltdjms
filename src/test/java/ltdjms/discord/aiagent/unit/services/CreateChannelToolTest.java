...
/**
 * 測試 {@link ltdjms.discord.aiagent.services.tools.CreateChannelTool CreateChannelTool} 的工具執行邏輯。
 *
 * <p>測試範圍：
 *
 * <ul>
 *   <li>T025: CreateChannelTool 單元測試
 * </ul>
 *
 * <p>測試案例涵蓋：
 *
 ...
 * </ul>
 */
@DisplayName("T025: CreateChannelTool 單元測試")
class CreateChannelToolTest {
  ...
  private CreateChannelTool createChannelTool;
  ...
  void setUp() {
    ...
    toolContext = new ToolContext(TEST_GUILD_ID, TEST_CHANNEL_ID, TEST_USER_ID, mockJda);
    createChannelTool = new CreateChannelTool();
  ...
  class SuccessTests {
    ...
    void shouldSuccessfullyCreateChannel() {
      ...
      ToolExecutionResult result = createChannelTool.execute(parameters, toolContext);

      // Then - 驗證結果
      assertThat(result.success()).isTrue();
      assertThat(result.result()).isPresent();
      assertThat(result.result().get()).contains(String.valueOf(TEST_CHANNEL_ID));
      assertThat(result.result().get()).contains(channelName);
    }
  ...
  class ParameterValidationTests {
    ...
    void shouldReturnErrorWhenNameIsEmpty() {
      ...
      assertThat(result.error().get()).contains("頻道名稱不能為空");
    }
    ...
    void shouldReturnErrorWhenNameExceedsMaxLength() {
      ...
      assertThat(result.success()).isFalse();
      assertThat(result.error()).isPresent();
      assertThat(result.error().get()).contains("頻道名稱不能超過 100 字符");
    }

    @Test
    @DisplayName("當頻道名稱剛好 100 字符時，應成功創建")
  ...
  class ErrorHandlingTests {
    ...
    void shouldReturnErrorWhenGuildNotFound() {
      ...
      Map<String, Object> parameters = new HashMap<>();
      parameters.put("name", "test-channel");
  ...
  class PermissionConfigurationTests {
    ...
    void shouldParseStringPermissionDescription() {
      // Given
      String channelName = "descriptive-permission-channel";
      Role mockRole = mock(Role.class);
      when(mockRole.getIdLong()).thenReturn(TEST_ROLE_ID);
      when(mockGuild.getRoleById(TEST_ROLE_ID)).thenReturn(mockRole);
