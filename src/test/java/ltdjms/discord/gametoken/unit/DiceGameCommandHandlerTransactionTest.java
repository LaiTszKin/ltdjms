...
/**
 * Command handler tests focusing on game token transaction recording. Verifies that playing dice
 * games creates game token transaction records, so they can appear in the user panel token history.
 */
class DiceGameCommandHandlerTransactionTest {
  ...
  class DiceGame1Tests {
    ...
    void shouldRecordTransactionWhenGame1Played() {
      ...
      GuildCurrencyConfigRepository currencyConfigRepository =
          mock(GuildCurrencyConfigRepository.class);
      ...
      DiceGameResult gameResult =
          new DiceGameResult(
              TEST_GUILD_ID, TEST_USER_ID, List.of(1, 2, 3, 4, 5), 1_000_000L, 0L, 1_000_000L);
      when(diceGameService.play(
              eq(TEST_GUILD_ID), eq(TEST_USER_ID), any(DiceGame1Config.class), anyInt()))
          .thenReturn(gameResult);

      when(currencyConfigRepository.findByGuildId(TEST_GUILD_ID))
          .thenReturn(
              Optional.of(
                  new GuildCurrencyConfig(
                      TEST_GUILD_ID, "Gold", "💰", Instant.now(), Instant.now())));

      DiceGame1CommandHandler handler =
          new DiceGame1CommandHandler(
              tokenService,
              diceGameService,
              configRepository,
    ...
    void shouldUsePlayerSpecifiedTokensWhenValid() {
      ...
      SlashCommandInteractionEvent event =
          createMockSlashEventWithTokens("dice-game-1", playerSpecifiedTokens);
    ...
    void shouldRejectTokensBelowMinimum() {
      ...
      verify(event).reply(argThat((String msg) -> msg.contains("5") && msg.contains("10")));
    }
    ...
    void shouldRejectMissingTokensWithErrorMessage() {
      ...
      verify(event).reply(argThat((String msg) -> msg.contains("1") && msg.contains("10")));
    }
    ...
    void shouldRollCorrectDiceCountBasedOnTokens() {
      ...
      when(diceGameService.play(
              eq(TEST_GUILD_ID),
              eq(TEST_USER_ID),
              any(DiceGame1Config.class),
              eq((int) tokensToSpend)))
          .thenReturn(gameResult);
      ...
      SlashCommandInteractionEvent event =
          createMockSlashEventWithTokens("dice-game-1", tokensToSpend);
  ...
  class DiceGame2Tests {
    ...
    void shouldRecordTransactionWhenGame2Played() {
      ...
      DiceGame2Result gameResult =
          new DiceGame2Result(
              ...
              0L);
      when(diceGameService.play(
              eq(TEST_GUILD_ID), eq(TEST_USER_ID), any(DiceGame2Config.class), anyInt()))
          .thenReturn(gameResult);

      when(currencyConfigRepository.findByGuildId(TEST_GUILD_ID)).thenReturn(Optional.empty());

      DiceGame2CommandHandler handler =
          new DiceGame2CommandHandler(
              tokenService,
              diceGameService,
              configRepository,
              currencyConfigRepository,
    ...
    void shouldUsePlayerSpecifiedTokensWhenValid() {
      ...
      SlashCommandInteractionEvent event =
          createMockSlashEventWithTokens("dice-game-2", playerSpecifiedTokens);
    ...
    void shouldRejectTokensBelowMinimum() {
      ...
      verifyNoInteractions(diceGameService);
      // Warning message sent as ephemeral reply
      verify(event).reply(argThat((String msg) -> msg.contains("5") && msg.contains("50")));
    }
  ...
  private SlashCommandInteractionEvent createMockSlashEventWithTokens(
      String commandName, long tokens) {
    SlashCommandInteractionEvent event = mock(SlashCommandInteractionEvent.class);
    Guild guild = mock(Guild.class);
    User user = mock(User.class);
    ReplyCallbackAction replyAction = mock(ReplyCallbackAction.class);
