...
class AdminPanelServiceEventTest {
    ...
    void updateDiceGame2Config_shouldPublishEventOnSuccess() {
        ...
        long guildId = 123L;
        Instant now = Instant.now();
        DiceGame2Config defaultConfig = new DiceGame2Config(guildId, 1L, 10L, 5L, 2L, 50L, 100L, now, now);
        DiceGame2Config updatedConfig = new DiceGame2Config(guildId, 5L, 20L, 10L, 3L, 75L, 150L, now, now);
        ...
        // When
        Result<DiceGame2Config, ?> result = service.updateDiceGame2Config(
                guildId, 5L, 20L, 10L, 3L, 75L, 150L);
