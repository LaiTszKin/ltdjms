 ...
 *
 * <p>此測試定義 DiscordSessionManager 介面的行為契約。 所有實作都必須符合此契約。
 */
...
class DiscordSessionManagerTest {
  ...
  private static class TestDiscordSessionManager implements DiscordSessionManager<TestSessionType> {
    private final Map<String, DiscordSessionManager.Session<TestSessionType>> sessions =
    ...
    public void registerSession(
      ...
      DiscordSessionManager.Session<TestSessionType> session =
          new DiscordSessionManager.Session<>(type, hook, Instant.now(), 900, metadata);
      sessions.put(key, session);
    }

    @Override
    public Optional<DiscordSessionManager.Session<TestSessionType>> getSession(
        TestSessionType type, long guildId, long userId) {
      String key = getKey(type, guildId, userId);
  ...
  void sessionIsExpiredShouldCorrectlyDetermineExpiration() {
    ...
    // 未過期的 Session
    DiscordSessionManager.Session<TestSessionType> validSession =
        new DiscordSessionManager.Session<>(
            TestSessionType.USER_PANEL, hook, Instant.now(), 900, Map.of());

    // 已過期的 Session
    DiscordSessionManager.Session<TestSessionType> expiredSession =
        new DiscordSessionManager.Session<>(
            TestSessionType.USER_PANEL, hook, Instant.now().minusSeconds(1000), 900, Map.of());
  ...
  void sessionRecordShouldContainAllRequiredInfo() {
    ...
    DiscordSessionManager.Session<TestSessionType> session =
        new DiscordSessionManager.Session<>(
            TestSessionType.USER_PANEL, hook, Instant.now(), 900, metadata);
  ...
  void interfaceMethodSignaturesShouldBeCorrect() {
    assertThat(DiscordSessionManager.class)
        .hasDeclaredMethods(
            "registerSession", "getSession", "clearSession", "clearExpiredSessions");
  }
}
