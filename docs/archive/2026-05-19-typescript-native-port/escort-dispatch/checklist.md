...
- [x] CL-01: EscortDispatchOrder 7 狀態機所有合法轉換路徑 — R1–R9 → UT-Dispatch-01 — Result: `PASS`
...
- [x] CL-02: 所有非法狀態轉換被正確拒絕 — R1–R9 → UT-Dispatch-02 — Result: `PASS`
  - 已確認的訂單不可重複確認
  - 非護航者不可確認/完單
  - 非客戶不可確認完成/申請售後
  - 已在售後流程中的訂單不可重複申請售後
...
- [x] CL-03: EscortDispatchOrder 建構驗證（22 欄位、escort ≠ customer、source type 對應 snapshot 規則）— R1.3–R1.4 → UT-Dispatch-03 — Result: `PASS`
...
- [x] CL-04: Handoff from currency purchase 的 idempotency（findBySourceIdentity）— R11.1–R11.7 → UT-Handoff-01 — Result: `PASS`
  - 同一 sourceType + sourceReference 只建立一筆訂單
  - 第二次呼叫直接回傳既有訂單

- [x] CL-05: Handoff from fiat payment 的 idempotency + snapshot survival — R11.2–R11.7 → UT-Handoff-02 — Result: `PASS`
...
- [x] CL-06: Handoff exception fallback（create 失敗後的 findBySourceIdentity 重試）— R11.7 → UT-Handoff-03 — Result: `PASS`
  - save 失敗 → catch block 內再次查詢 → 若有則回傳既有訂單，無則回傳 persistenceFailure

- [x] CL-07: assignEscort 並發控制 — R3.2 → IT-Repo-01 — Result: `PASS`
  - 兩個 assignEscort 同時執行，只有一個成功（WHERE escort_user_id=0 RETURNING *）
  - 第二個回傳 null

- [x] CL-08: claimAfterSales 並發控制 — R8.2 → IT-Repo-02 — Result: `PASS`
  - 兩個 claimAfterSales 同時執行，只有一個成功（WHERE after_sales_assignee_user_id IS NULL RETURNING *）
  - 第二個回傳 null → service 層解讀為「已被其他售後人員接手」

- [x] CL-09: closeAfterSales 並發控制 — R9.2 → IT-Repo-03 — Result: `PASS`
  - 非接手人的 closeAfterSales 回傳 null

- [x] CL-10: 訂單編號唯一性重試機制 — R1.2 → UT-Number-01 — Result: `PASS`
  - 最多重試 20 次，超過拋出例外

### 超時自動完成 (Timeout Auto-Completion)

- [x] CL-11: 24h 客戶確認超時自動完成 — R10.1–R10.3 → UT-Timeout-01 — Result: `PASS`
  - completionRequestedAt + 24h < now → 自動 COMPLETED
  - 自動完成僅在查詢時觸發（lazy），非背景排程
  - 自動完成失敗（DB error）時回傳原始訂單，不拋出例外

- [x] CL-12: 超時邊界條件 — R6.3, R10.2 → UT-Timeout-02 — Result: `PASS`
  - 恰好 24h 時視為已超時（非嚴格小於）
  - 未達 24h 不觸發自動完成

- [x] CL-13: 客戶確認完成時的 idempotent（已 COMPLETED 狀態）— R6.2 → UT-Timeout-03 — Result: `PASS`
  - 若 ensureTimeoutCompletion 已自動完成，customerConfirmCompletion 直接回傳 ok

### 通知流程 (Notification Flow)

- [x] CL-14: DM 通知完整鏈路 — R3.4–R9.3 → UT-Notify-01 — Result: `PASS`
...
- [x] CL-15: DM 發送失敗不阻斷業務流程 — R3.4 → UT-Notify-02 — Result: `PASS`
  - 用戶關閉私訊 → 記錄 warn log → 管理員操作仍視為成功
  - 用戶不存在（retrieveUser 失敗）→ 記錄 warn log → 不拋出例外

- [x] CL-16: 售後通知在線優先策略 — R7.4 → UT-Notify-03 — Result: `PASS`
  - 有在線售後人員 → 僅通知在線者
  - 無在線售後人員 → 通知全部設定人員
  - 無設定售後人員 → 回傳提示訊息
...
- [x] CL-17: 派單面板完整互動流程 — R14 → IT-Panel-01 — Result: `PASS`
...
- [x] CL-23: 售後人員管理 — R13 → UT-Staff-01 — Result: `PASS`
...
- [x] CL-24: 訂單編號格式 ESC-YYYYMMDD-XXXXXX — R1.1 → UT-Number-02 — Result: `PASS`
  - 日期部分正確（YYYYMMDD）
...
- [x] CL-25: 護航品類驗證 — R2.1 → UT-Service-01 — Result: `PASS`
  - 有效的 option code（如 CONF_DAM_300W）→ pass
  - 無效的 option code → DomainError("護航品類無效，可用代碼：...")

- [x] CL-26: 護航品類 select menu 自動分頁 — R2.3 → UT-View-01 — Result: `PASS`
  - ≤25 個選項 → 單一 select
  - >25 個選項 → SELECT_ORDER_OPTION + SELECT_ORDER_OPTION_EXTRA

- [x] CL-27: 待派單訂單清單為空 — R3.1 edge case → UT-View-02 — Result: `PASS`
  - pendingOrders 為空 → select menu disabled，顯示「目前沒有待派單訂單」

- [x] CL-28: 客戶已不在伺服器中 — R14 create flow edge case → UT-Panel-05 — Result: `PASS`
  - retrieveMemberById 失敗 → 回傳「找不到指定客戶，請確認該成員仍在伺服器中」

## Hardening Checklist

- [x] Regression tests for bug-prone/high-risk behavior: 狀態機轉換、條件式 UPDATE race condition、handoff idempotency
- [x] Unit drift checks for non-trivial tasks: 所有 withXxx 方法回傳正確 status；ensureTimeoutCompletion 在臨界時間點行為正確
- [x] Property-based coverage for business logic: EscortDispatchOrder 建構驗證（任意 22 欄位組合 → 驗證規則不變）；訂單編號字元集（任意尾碼 → 無混淆字元）
- [x] External services mocked/faked: DiscordRuntimeGateway（通知測試）、Database（repository 單元測試）、EscortOptionCatalogRepository（定價測試）
- [x] Adversarial cases for abuse paths:
...
- [x] Authorization, idempotency, concurrency risks evaluated:
...
- [x] Fixtures reproducible (fixed seed/clock):
  - Clock injection 用於 timeout 測試
  - 訂單編號產生器接受 Clock + Random seed injection

## E2E / Integration Decisions

- [x] 完整派單生命週期 E2E: 對接真實 PostgreSQL + Discord bot staging → 驗證 /dispatch-panel → create → assign → confirm → complete 完整流程 — Reason: 確保 TypeScript 版本行為與 Java production 一致
- [x] Handoff 整合測試: 使用真實 PostgreSQL → 驗證 idempotency + snapshot survival → Reason: handoff 是跨模組邊界，需要真實 DB 驗證 conditional logic
- [x] Notification E2E: 使用 Discord staging bot → 驗證所有 6 種 DM 通知的 embed 格式與按鈕互動 → Reason: DM 格式是使用者可見行為，必須與 Java 版完全一致
- [x] Concurrent assignEscort 整合測試: 使用真實 PostgreSQL → 兩個同時 UPDATE → 只有一個成功 → Reason: 確保 WHERE guard 在 PostgreSQL 層級正確
- [x] Concurrent claimAfterSales 整合測試: 使用真實 PostgreSQL → 同上
...
- [x] Domain Model + State Machine: `COMPLETED` — Remaining: None
- [x] Repository (conditional UPDATE): `COMPLETED` — Remaining: None
- [x] Core Service (lifecycle + handoff): `COMPLETED` — Remaining: None
- [x] Notification Service (DM flows): `COMPLETED` — Remaining: None
- [x] Panel Interaction (slash command + buttons + session): `COMPLETED` — Remaining: None
- [x] Pricing + Staff Management: `COMPLETED` — Remaining: None
- [x] DI Registration + Package Setup: `COMPLETED` — Remaining: None
