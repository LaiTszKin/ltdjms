...
**Requirements**:
- [x] R1.1 `DiceGameMessages` 常數更新：GAME_1_TITLE 改為「骰子遊戲結果」
- [x] R1.2 `DiceGameMessages` 常數更新：GAME_1_RESULT 改為匹配 Java 格式（骰子結果 / 總獎勵 / 新餘額）
- [x] R1.3 `DiceGameMessages` 移除不再使用的 GAME_1_DESCRIPTION
- [x] R1.4 `DiceGame1Handler` 訊息組裝邏輯更新：移除 diceSum、餘額變動、描述 footer
- [x] R1.5 `DiceGame1Handler` 獎勵/餘額數字加入千分位格式化
- [x] R1.6 `DiceGame1Handler` 顯示 currencyIcon + currencyName（已在 handler 內取得 currencyConfig）
...
**Requirements**:
- [x] R2.1 `DiceGameMessages` 常數更新：GAME_2_TITLE 改為「骰子遊戲2結果」
- [x] R2.2 `DiceGameMessages` 常數更新：GAME_2_RESULT 改為匹配 Java 格式
- [x] R2.3 `DiceGameMessages` 移除不再使用的 GAME_2_DESCRIPTION
- [x] R2.4 `DiceGame2Handler` 訊息組裝邏輯更新：移除 segment 顯示、餘額變動
- [x] R2.5 `DiceGame2Handler` 三條區段加入組數顯示（`tripleSegments.length`）
- [x] R2.6 `DiceGame2Handler` 順子/三條/基礎獎勵僅在有值時顯示（條件渲染）
- [x] R2.7 `DiceGame2Handler` 所有獎勵/餘額數字加入千分位格式化
- [x] R2.8 `DiceGame2Handler` 總獎勵和新餘額使用粗體
...
**Requirements**:
- [x] R3.1 未輸入代幣數量時，顯示：「請輸入本局要投入的遊戲代幣數量！\n必須介於 X ~ Y 代幣之間」（含實際 min/max）
- [x] R3.2 代幣數量低於最低限制時，顯示：「代幣投入數量超出範圍！\n您輸入的數量：X\n允許範圍：min ~ max 代幣」
- [x] R3.3 代幣數量高於最高限制時，顯示：同上 R3.2 格式
- [x] R3.4 代幣不足時，顯示：「遊戲代幣不足！\n需要：X 代幣\n目前餘額：Y 代幣」（含千分位格式化）
- [x] R3.5 `DiceGameMessages` 常數更新：移除 INVALID_TOKEN_COUNT、TOKEN_COUNT_TOO_LOW、TOKEN_COUNT_TOO_HIGH、TOKEN_INSUFFICIENT，替換為含參數的模板字串
- [x] R3.6 `DiceGame1Handler` 和 `DiceGame2Handler` 更新錯誤處理分支使用新訊息格式

## Error and Edge Cases
- [x] 當 currencyConfig 查詢失敗時，遊戲結果仍能顯示（使用預設貨幣名稱/icon）
- [x] 當骰子值非 1-6 時，emoji mapping 有 fallback 顯示數字
- [x] 代幣數量為 0 或負數時的邊界處理（現有行為保留：視為無效輸入）
