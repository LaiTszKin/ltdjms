...
**Requirements**:
- [x] R1.1 Mirror Java `ShopView` public constants
- [x] R1.2 Search modal text input customId = `keyword`（非 `shop_search_input`）
- [x] R1.3 Parity test 比對 `fixtures/java-shop-custom-ids.json`
...
**Requirements**:
- [x] R2.1 Button labels `⬅️ 上一頁` / `下一頁 ➡️`
- [x] R2.2 空商店 embed 文案對齊 Java
- [x] R2.3 UT snapshot 對齊 `fixtures/java-shop-view-oracle.json` browse 場景
...
**Requirements**:
- [x] R3.1 修正 TS off-by-one（handlers 傳 `page - 1` 或等價）
- [x] R3.2 新增 `hasProducts(guildId)`、`getProductCount(guildId)`
- [x] R3.3 Port `ShopServiceTest.java` → vitest
...
**Requirements**:
- [x] R4.1 `/shop` 初始 page index 0
- [x] R4.2 Guild guard：`此功能只能在伺服器中使用`
- [x] R4.3 Search empty：`找不到符合「{keyword}」的商品`
- [x] R4.4 Buy 按鈕使用 `productService.getAllPurchasableProducts(guildId)`
- [x] R4.5 Port `ShopButtonHandlerTest.java`
...
**Requirements**:
- [x] R5.1 Currency-only → confirm embed + `shop_confirm_purchase_*` / `shop_cancel_purchase`
- [x] R5.2 Fiat-only → deferReply + DM payment + interaction summary
- [x] R5.3 Dual-price → payment choice（`💰 貨幣購買` / `💳 法幣下單`，無價格在 label）
- [x] R5.4 Inflight fiat dedup（ConcurrentHashMap 等價）
- [x] R5.5 Currency purchase 後 escort handoff + buyer/admin notify（對齊 Java handler）
- [x] R5.6 Port `ShopSelectMenuHandlerTest.java`
...
**Requirements**:
- [x] R6.1 Buy menu 描述格式對齊 Java（combined price description）
- [x] R6.2 Search result component order：buy select → pagination → back
- [x] R6.3 Search pagination 始終顯示 prev/next（disabled at bounds）

## Error and Edge Cases
- [x] 分頁超出範圍 — clamp 至有效頁
- [x] 餘額不足 — confirm embed 顯示錯誤或禁止 confirm（對齊 Java）
- [x] 重複 fiat 訂單 — inflight guard ephemeral 提示
- [x] DM 關閉 — fiat fallback 訊息對齊 Java
- [x] 非 guild interaction — 統一錯誤文案
