...
| `INT-009` | Session 管理：建立、查詢、TTL 過期清理                                                          | `admin-panel` / `user-panel` → `@ltdjms/shared` | sync call：`DiscordSessionManager` 介面                                                                                                  | session key（guildId+userId）、session state、InteractionHook reference → session 物件                                  | Shared module 提供 session 基礎設施；TTL 過期後自動清理                                                  |
...
| Session 使用 `@ltdjms/shared` 的 `DiscordSessionManager`（基於 Redis + in-memory fallback）| 自行實作獨立 session 管理                                          | Session key 格式、TTL 機制與其他 module 一致；Redis 不可用時自動降級                                               |
