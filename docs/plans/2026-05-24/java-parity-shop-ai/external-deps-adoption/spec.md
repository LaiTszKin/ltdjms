...
**Requirements**:
- [ ] R1.1 `packages/ai/package.json` 新增 LangGraph 生態 runtime deps
- [ ] R1.2 `packages/shop/package.json` 新增 mock/supertest devDeps
- [ ] R1.3 `pnpm install` 更新 lockfile 且無 peer dependency 衝突
...
**Requirements**:
- [ ] R2.1 建立 `packages/shared/src/__tests__/parity/json-snapshot.ts`（或等價位置）export `assertJsonParity(actual, oracle)`
- [ ] R2.2 文件化慣例：Java fixture → TS `toMatchJsonSnapshot()` 流程
...
**Requirements**:
- [ ] R3.1 Postgres checkpoint 整合測試通過（testcontainers 或現有 test DB）
- [ ] R3.2 Redis checkpoint 整合測試通過或明確記錄 fallback（僅 Postgres）
- [ ] R3.3 PoC 報告：是否保留 REASONING/TOOL_INTENT 外層 streaming 控制
...
**Requirements**:
- [ ] R4.1 至少 2 個 tool schema PoC 測試通過
- [ ] R4.2 與 `@langchain/core` `StructuredTool` binding 相容
...
**Requirements**:
- [ ] R5.1 shop package 內 1 個 smoke test 通過
- [ ] R5.2 若 pre-release 不穩，document fallback hand mock 策略
...
**Requirements**:
- [ ] R6.1 1 個 callback route smoke test 通過
- [ ] R6.2 與現有 `payment-callback.test.ts` 不衝突

## Error and Edge Cases
- [ ] LangGraph peer dependency 與 `@langchain/core@1.1.48` 不相容 → 記錄並 pin 相容版本
- [ ] @robojs/mock 與 discord.js 14 型別衝突 → 使用 `@ts-expect-error` 或 hand mock fallback
- [ ] Redis Stack 本機不可用 → ai-agent spec 改 Postgres-only checkpoint
- [ ] supertest 與 Express 5 middleware 型別 → 使用 `@types/supertest@^6`
