...
- T1.1 [x] **packages/ai 新增 LangGraph deps** — `@langchain/langgraph@^1.3.2`, checkpoint-postgres, checkpoint-redis, `zod-to-json-schema@^3.25.2`
  - Verify: `pnpm --filter @ltdjms/ai list @langchain/langgraph`

- T1.2 [x] **packages/shop 新增 test deps** — `@robojs/mock@0.1.1-next.1`, `supertest@^7.2.2`, `@types/supertest@^6.0.0`
  - Verify: `pnpm --filter @ltdjms/shop list @robojs/mock`

- T1.3 [x] **pnpm install + make build**
  - Verify: exit code 0
...
- T2.1 [x] **建立 json-snapshot helper** — `assertJsonParity`, `normalizeEmbedForSnapshot`
  - Verify: 單元測試 helper 本身通過

- T2.2 [x] **README 片段** — 在 batch preparation 或 shared test docs 記錄 Java fixture → snapshot 流程
...
- T3.1 [x] **Postgres checkpoint write/read PoC**
  - Verify: `pnpm vitest run --project @ltdjms/ai -t "postgres checkpoint"`

- T3.2 [x] **Redis checkpoint write/read PoC**（或 document Postgres-only fallback）
  - Verify: PoC test 綠或 `design.md` 記錄 fallback

- T3.3 [x] **PoC 結論寫入 design.md** — streaming 外層保留策略
...
- T4.1 [x] **create_channel + list_channels schema PoC**
  - Verify: generated JSON schema 含 required fields

- T4.2 [x] **StructuredTool.fromZod 或等價 binding 驗證**
  - Verify: PoC test 綠
...
- T5.1 [x] **Mock /shop slash interaction smoke**
...
- T6.1 [x] **Callback route smoke with supertest**
