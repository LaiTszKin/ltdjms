...
- T1.1 [x] **bump shared dependencies** — zod 4、drizzle 0.45、pino 10、discord.js 14.26、pg 8.21、ioredis 5.10
  - Verify: `pnpm --filter @ltdjms/shared exec tsc --noEmit`

- T1.2 [x] **修復 shared zod schemas** — 適配 zod 4 API
  - Verify: `pnpm vitest run --project @ltdjms/shared`

- T1.3 [x] **修復 shared logger** — 適配 pino 10
  - Verify: shared 測試通過
...
- T2.1 [x] **bump + fix economy** — zod schema、 drizzle query
  - Verify: `pnpm vitest run --project @ltdjms/economy`

- T2.2 [x] **bump + fix games** — 同上
  - Verify: `pnpm vitest run --project @ltdjms/games`

- T2.3 [x] **bump + fix dispatch** — reflect-metadata 改 runtime dep
...
- T3.1 [x] **bump + fix shop** — zod + pino（express 留給 express spec）
  - Verify: shop 測試通過

- T3.2 [x] **bump + fix admin + ai + bot**
  - Verify: `make verify`
...
- T4.1 [x] **統一 pino/pg/reflect-metadata 版本** — 消除 package 間不一致
