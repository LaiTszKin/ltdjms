...
- T1.1 [ ] **更新根 package.json** — bump `typescript@^6.0.3`、`vitest@^4.1.7`、`eslint@^10.4.0`、`typescript-eslint@^8.59.4`、`prettier@^3.8.3`、`tsx@^4.22.3`
  - Verify: `node -p "require('./package.json').devDependencies.typescript"`

- T1.2 [ ] **pnpm install** — 更新 lockfile
...
- T2.1 [ ] **更新所有 package typescript + vitest** — 每個 package devDependencies 對齊根版本
  - Verify: `grep -r '"typescript":' packages/*/package.json | sort -u` 僅一行 `^6.0.3`

- T2.2 [ ] **更新所有 package @types/node** — 改為 `^22.0.0`
...
- T3.1 [ ] **執行 make build 並修復錯誤** — 逐 package 修復編譯 error
...
- T4.1 [ ] **更新 vitest workspace config** — 適配 Vitest 4 projects/pool API
  - Verify: `make test` exit code 0
...
- T5.1 [ ] **更新 eslint.config.mjs** — 適配 ESLint 10 flat config
  - Verify: `make lint` exit code 0
...
- T6.1 [ ] **確認 CI 使用 Node 22** — 與 preparation 一致
