...
- [x] T1.1 **`packages/shared/src/localization/command-localizations.ts`** — 建立 `CommandLocalizations` 物件/類別：
...
- [x] T1.2 **`packages/shared/src/localization/index.ts`** — 匯出 `CommandLocalizations`
  - Verify: import 路徑正確 ✅
...
- [x] T2.1 **`packages/shared/src/localization/dice-game-messages.ts`** — 建立 `DiceGameMessages` 模組：
...
- [x] T3.1 **`packages/economy/src/commands/*.ts`** — 更新所有指令 handler 的 import：
...
- [x] T3.2 **`packages/admin/src/panel/admin/definitions/`** — 更新管理面板指令定義的在地化引用
  - AdminPanelSlashCommand.ts, UserPanelSlashCommand.ts ✅
  - EconomySlashCommands.ts, DispatchSlashCommands.ts ✅
  - SlashCommandRegistrar.ts (ShopSlashCommand) ✅
  - Verify: `make build` 通過 ✅

- [x] T3.3 **刪除重複的在地化檔案** — 若 economy 模組內有獨立的地化檔案，確認 shared 版本內容一致後刪除
  - `packages/economy/src/localization/dice-game-messages.ts` 已刪除 ✅
  - Verify: `make build` 通過; 無 broken import ✅

- [x] T3.4 **`packages/shared/src/index.ts`** — 從 shared 公開 API 匯出 localiz ation 模組
  - Verify: `make build` 通過 ✅
