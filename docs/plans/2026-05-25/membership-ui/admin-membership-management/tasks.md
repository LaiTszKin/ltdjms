...
- T1.1 [x] **`MembershipSpendRepository`** — 新增 `insertAdminAdjust(...)` 介面；常數 `SOURCE_ADMIN_ADJUST = "ADMIN_ADJUST"`
  - Verify: compile

- T1.2 [x] **JDBC impl** — INSERT with unique source_reference；复用既有 ON CONFLICT DO NOTHING 或 throw on duplicate
...
- T2.1 [x] **`MembershipAdminService.adjustPeriodSpend`** — ADD/DEDUCT/SET delta 計算 + insert
  - Verify: UT-01, UT-02

- T2.2 [x] **`MembershipAdminService.setTier`** — update membership row + bronze flag + event
  - Verify: UT-03, UT-04

- T2.3 [x] **`MembershipAdminService.getDetail`** — compose admin detail DTO
...
- T3.1 [x] **`MembershipManagementFacade`** — delegate read/write to admin service
  - Verify: compile + facade unit test

- T3.2 [x] **`AdminPanelService`** — 暴露三個 membership 方法
  - Verify: compile

- T3.3 [x] **`CommandHandlerModule`** — wire facade + admin service
  - Verify: `make build`
...
- T4.1 [x] **Main menu** — 新增「🏅 會員等級管理」按鈕與 customId 路由
  - Verify: AdminPanelCommandHandlerTest 若存在

- T4.2 [x] **User select + detail embed** — 顯示 R1.2 欄位
  - Verify: manual admin panel

- T4.3 [x] **Adjust spend flow** — mode select + modal + submit handler
  - Verify: integration test or handler unit test with mocks

- T4.4 [x] **Set tier flow** — tier StringSelect + confirm
...
- T5.1 [x] **administration.md** — 會員等級管理章節
  - Verify: 對照 spec

- T5.2 [x] **`make verify`** — 全專案綠
  - Verify: `make verify`（2061/2062 pass；`FiatOrderServiceTest` 既有 shop 文案失敗，非本 feature 引入）
