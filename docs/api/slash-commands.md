...
所有工具調用都會記錄到資料庫，但只保存去敏摘要，包含：
- 觸發用戶 ID
- 執行時間
- 工具名稱，以及參數／結果／錯誤內容的摘要與雜湊
- 執行結果（成功/失敗）
...
**行為模式**：
- **預設拒絕**（預設）：未設定任何頻道時，AI 不會在任何頻道回應
- **允許清單模式**：設定允許頻道清單後，AI 僅在清單中的頻道回應
...
2. 從選單中選擇要移除的頻道
3. 成功後顯示確認訊息
4. 若清空所有頻道，會恢復為空 allowlist，AI 將不再回應任何頻道
