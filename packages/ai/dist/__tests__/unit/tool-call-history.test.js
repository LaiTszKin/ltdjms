...
import { InMemoryToolCallHistory, ConversationIdBuilder, } from '../../services/memory/tool-call-history.js';
...
describe('InMemoryToolCallHistory', () => {
    ...
    it('should create REDACTED memory summary for search operations', () => {
        const { memorySummary, redactionMode } = InMemoryToolCallHistory.createMemorySummary('search_messages', { keywords: 'important' }, 'Found some results');
    ...
    it('should redact Discord URLs in memory summaries', () => {
        const { memorySummary, redactionMode } = InMemoryToolCallHistory.createMemorySummary('send_messages', {}, 'Sent message to https://discord.com/channels/123/456');
        expect(redactionMode).toBe(RedactionMode.REDACTED);
...
describe('ConversationIdBuilder', () => {
    ...
    it('should detect thread-level strategy', () => {
        expect(ConversationIdBuilder.parseStrategy('guild-1:thread-1:user-1')).toBe(ConversationIdStrategy.THREAD_LEVEL);
    });
    it('should detect message-level strategy', () => {
        expect(ConversationIdBuilder.parseStrategy('guild-1:channel-1:user-1:msg-1')).toBe(ConversationIdStrategy.MESSAGE_LEVEL);
    });
...
//# sourceMappingURL=tool-call-history.test.js.map