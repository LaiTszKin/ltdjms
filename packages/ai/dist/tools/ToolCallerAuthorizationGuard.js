...
export class ToolCallerAuthorizationGuard {
     ...
     */
    async validateAdministrator(guild, toolName) {
        ...
        try {
            ...
            console.warn(`[Auth] Non-admin user ${userId} attempted to use tool ${toolName} in guild ${guild.id}`);
            return '你沒有權限使用此工具。';
        }
        catch {
            console.warn(`[Auth] Member not found for user ${userId} in guild ${guild.id} attempting tool ${toolName}`);
            return '無法在伺服器中找到你的成員資料。';
        }
    }
}
//# sourceMappingURL=ToolCallerAuthorizationGuard.js.map