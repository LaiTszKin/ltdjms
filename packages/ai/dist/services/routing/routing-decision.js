 ...
 */
export function resolveRestrictionChannelId(channel) {
    if (channel.isThread()) {
 ...
 */
export function resolveCategoryId(channel, guild) {
...
export class AIChatMentionRoutingDecision {
    ...
    constructor(agentConfigService, channelRestrictionService) {
     ...
     */
    async decide(guildId, channelId, restrictionChannelId, categoryId) {
        ...
        const channelAllowed = await this.channelRestrictionService.isChannelAllowed(guildId, restrictionChannelId, categoryId ?? undefined);
        if (channelAllowed) {
            return {
