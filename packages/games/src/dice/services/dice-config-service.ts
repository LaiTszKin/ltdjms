...
export class DiceConfigService {
   ...
   */
  async upsertDice1Config(
    config: DiceGame1Config,
    previousVersion?: DiceGame1Config | null,
  ): Promise<DiceGame1Config> {
    const oldConfig =
      previousVersion ?? (await this.diceConfigRepository.findDice1Config(config.guildId));
   ...
   * @param previousVersion - pre-fetched old config to avoid redundant DB query (P2-5)
   */
  async upsertDice2Config(
    config: DiceGame2Config,
    previousVersion?: DiceGame2Config | null,
  ): Promise<DiceGame2Config> {
    const oldConfig =
      previousVersion ?? (await this.diceConfigRepository.findDice2Config(config.guildId));
