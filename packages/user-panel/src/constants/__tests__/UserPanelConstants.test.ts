...
describe('UserPanelConstants (UT-204)', () => {
  ...
  it('should use user_panel routing prefix', () => {
    expect(UserPanelConstants.ROUTING_PREFIX).toBe(oracle.prefix);
    expect(UserPanelConstants.BUTTON_REDEEM.startsWith(UserPanelConstants.ROUTING_PREFIX)).toBe(
      true,
    );
    expect(UserPanelConstants.MODAL_REDEEM.startsWith(UserPanelConstants.ROUTING_PREFIX)).toBe(
      true,
    );
  });
});
