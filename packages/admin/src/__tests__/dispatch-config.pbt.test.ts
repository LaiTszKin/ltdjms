...
describe('DispatchManagementFacade PBT', () => {
  ...
  beforeEach(() => {
    ...
    mockPrice = {
      listOptionPrices: vi.fn(),
      updateOptionPrice: vi.fn(),
      resetOptionPrice: vi.fn(),
    };
    mockCat = {
      findAll: vi.fn(),
      findByCode: vi.fn(),
      ...
      update: vi.fn(),
      delete: vi.fn(),
      countByOptionCode: vi.fn(),
      findGuildIdsByOptionCode: vi.fn(),
    };
    mockOrder = { countActiveOrders: vi.fn() };
    mockEvt = { publish: vi.fn() };
    facade = new DispatchManagementFacade(
      mockStaff as DispatchAfterSalesStaffService,
      mockPrice as EscortOptionPricingService,
      mockCat as EscortCatalogService,
      mockEvt as DomainEventPublisher,
      mockOrder as EscortDispatchOrderService,
    );
  });
  ...
  describe('售後人員', () => {
    it('listStaff 委派並回傳集合', async () => {
      await fc.assert(
        fc.asyncProperty(guildId(), fc.array(userId()), async (gId, ids) => {
      ...
      await fc.assert(
        fc.asyncProperty(guildId(), userId(), async (gId, uId) => {
          ...
          expect(mockEvt.publish).toHaveBeenLastCalledWith(
            expect.objectContaining({ eventType: 'dispatch_after_sales_config_changed' }),
          );
          return true;
        }),
      );
    ...
    it('addStaff 失敗時不發布事件', async () => {
      await fc.assert(
        fc.asyncProperty(guildId(), userId(), async (gId, uId) => {
          mockStaff.addStaff = vi
            .fn()
            .mockResolvedValue(new Err(DomainError.invalidInput('已在名單中')));
          const r = await facade.addStaff(String(gId), String(uId));
  ...
  describe('訂單統計', () => {
    it('countActiveOrders 委派到 orderService', async () => {
      await fc.assert(
        fc.asyncProperty(guildId(), fc.integer({ min: 0, max: 1000 }), async (gId, cnt) => {
  ...
  describe('目錄管理', () => {
    ...
    it('findCatalogEntry 對任何 code 委派並回傳找到的條目', async () => {
      await fc.assert(
        fc.asyncProperty(fc.stringMatching(/^[a-z0-9]{1,10}$/), async (code) => {
          const entry: EscortOptionCatalogEntry = {
            code,
            type: 'CARRY',
            level: 'STANDARD',
            mapScope: 'ANY',
