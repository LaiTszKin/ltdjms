...
import type { GameTokenService, GameTokenTransactionService, TokenAdjustmentResult } from '@ltdjms/economy';
...
describe('GameTokenManagementFacade PBT', () => {
  ...
  beforeEach(() => {
    ...
    facade = new GameTokenManagementFacade(mockSvc as GameTokenService, mockTx as GameTokenTransactionService);
  });

  describe('getTokens', () => {
    it('delegate 到 tokenService', async () => {
      await fc.assert(fc.asyncProperty(guildId(), userId(), fc.integer({ min: 0, max: 1e5 }), async (gId, uId, bal) => {
  ...
  describe('adjustTokens', () => {
    it('有效金額被接受', async () => {
      await fc.assert(fc.asyncProperty(guildId(), userId(), fc.integer({ min: -1e5, max: 1e5 }), async (gId, uId, amt) => {
        const adj: TokenAdjustmentResult = { guildId: gId, userId: String(uId), previousTokens: 100, newTokens: 100 + amt, adjustment: amt };
        ...
        expect(r.isOk()).toBe(true);
        expect(mockSvc.tryAdjustTokens).toHaveBeenLastCalledWith(gId, String(uId), amt, expect.anything(), reason);
        return true;
      }));
    });
    it('NaN/Infinity 被拒絕', async () => {
      await fc.assert(fc.asyncProperty(guildId(), userId(), fc.constantFrom(NaN, Infinity, -Infinity), async (gId, uId, amt) => {
        const r = await facade.adjustTokens(String(gId), String(uId), amt, reason, actorId);
  ...
  describe('setTokens', () => {
    it('非負金額成功設定', async () => {
      await fc.assert(fc.asyncProperty(guildId(), userId(), fc.integer({ min: 0, max: 1e5 }), fc.integer({ min: 0, max: 1e5 }), async (gId, uId, cur, tgt) => {
        mockSvc.getBalance = vi.fn().mockResolvedValue(cur);
        const delta = tgt - cur;
        mockSvc.tryAdjustTokens = vi.fn().mockResolvedValue(new Ok({ guildId: gId, userId: String(uId), previousTokens: cur, newTokens: tgt, adjustment: delta }));
    ...
    it('負數金額被拒絕', async () => {
      await fc.assert(fc.asyncProperty(guildId(), userId(), fc.integer({ max: -1 }), async (gId, uId, amt) => {
  ...
  describe('getTokenTransactionPage', () => {
    it('delegate 到 transactionService', async () => {
      await fc.assert(fc.asyncProperty(guildId(), userId(), fc.integer({ min: 1, max: 10 }), fc.integer({ min: 5, max: 50 }), async (gId, uId, p, ps) => {
