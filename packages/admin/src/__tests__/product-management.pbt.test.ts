...
import type { ShopService, ProductRepository, RedemptionCodeRepository, RedemptionCodeGenerator, Product, RedemptionCode, CodeStats, ShopPage } from '@ltdjms/shop';
...
const makeProduct = (id: number, gId: number, name: string, price: number): Product => ({
  id, guildId: gId, name, description: null, rewardType: 'CURRENCY', rewardAmount: 1000,
  currencyPrice: price, fiatPriceTwd: null, autoCreateEscortOrder: false, escortOptionCode: null,
...
describe('ProductManagementFacade PBT', () => {
  ...
  beforeEach(() => {
    ...
    mockGen = { generate: vi.fn(() => { counter++; return `C-${String(counter).padStart(6, '0')}`; }) };
  ...
  describe('getShopPage', () => {
    it('delegate 到 shopService', async () => {
      await fc.assert(fc.asyncProperty(fc.integer({ min: 1, max: 1e5 }), fc.integer({ min: 1, max: 100 }), async (gId, page) => {
  ...
  describe('findProductById', () => {
    it('delegate 到 repository', async () => {
      await fc.assert(fc.asyncProperty(fc.integer({ min: 1, max: 1e5 }), async (id) => {
  ...
  describe('createProduct', () => {
    it('建立後發布 ProductChangedEvent', async () => {
      await fc.assert(fc.asyncProperty(guildId(), fc.string({ minLength: 1, maxLength: 20 }), fc.integer({ min: 1, max: 1e5 }), async (gId, name, price) => {
        const p = makeProduct(42, gId, name, price);
        mockRepo.create = vi.fn().mockResolvedValue(p);
        const data = { guildId: gId, name, description: null, rewardType: 'CURRENCY' as const, rewardAmount: 1000, currencyPrice: price, fiatPriceTwd: null, autoCreateEscortOrder: false, escortOptionCode: null };
        expect(await facade.createProduct(data, String(gId))).toEqual(p);
        expect(mockEvt.publish).toHaveBeenLastCalledWith(expect.objectContaining({ eventType: 'product_changed', productId: 42, operationType: OperationType.CREATED }));
        return true;
      }));
    });
  });
  ...
  describe('updateProduct', () => {
    it('更新後發布 ProductChangedEvent', async () => {
      await fc.assert(fc.asyncProperty(fc.integer({ min: 1, max: 1e5 }), guildId(), fc.string({ minLength: 1, maxLength: 50 }), async (id, gId, name) => {
        ...
        expect(mockEvt.publish).toHaveBeenLastCalledWith(expect.objectContaining({ eventType: 'product_changed', productId: id, operationType: OperationType.UPDATED }));
        return true;
      }));
    });
  });
  ...
  describe('deleteProduct', () => {
    it('成功刪除時發布事件', async () => {
      await fc.assert(fc.asyncProperty(fc.integer({ min: 1, max: 1e5 }), guildId(), async (id, gId) => {
        mockRepo.delete = vi.fn().mockResolvedValue(true);
        expect(await facade.deleteProduct(id, String(gId))).toBe(true);
        expect(mockEvt.publish).toHaveBeenLastCalledWith(expect.objectContaining({ eventType: 'product_changed', productId: id, operationType: OperationType.DELETED }));
        return true;
      }));
    });
  ...
  describe('generateAndSaveCodes', () => {
    it('生成的兌換碼數量等於請求的 count', async () => {
      await fc.assert(fc.asyncProperty(fc.integer({ min: 1, max: 100 }), guildId(), fc.integer({ min: 1, max: 1000 }), async (count, gId, prodId) => {
        counter = 0;
        mockGen.generate = vi.fn(() => { counter++; return `G-${String(counter).padStart(6, '0')}`; });
        const saved: RedemptionCode[] = Array.from({ length: count }, (_, i) => ({ id: i + 1, code: `G-${String(i + 1).padStart(6, '0')}`, guildId: gId, productId: prodId, quantity: 1, expiresAt: null, createdAt: new Date() }));
        ...
        expect(mockEvt.publish).toHaveBeenLastCalledWith(expect.objectContaining({ eventType: 'redemption_codes_generated', count }));
        return true;
      }));
    });
