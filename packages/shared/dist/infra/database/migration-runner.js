...
export async function runMigrations(pool, migrationsDir, logger) {
    ...
    for (let attempt = 1; attempt <= 3; attempt++) {
        try {
            ...
            if (!tableExists) {
                ...
                if (tableCount > 0) {
                    log.info(`__drizzle_migrations table not found but ${tableCount} tables exist — skipping migrate()`);
                    return;
                }
            }
    ...
    throw new SchemaMigrationException(`Database migration failed after 3 attempts: ${lastError?.message}`, lastError);
}
