...
describe('MockDiscordContext', () => {
    ...
    it('validates non-empty guild id', () => {
        expect(() => new MockDiscordContext('', '456', '789', '<@456>')).toThrow('guildId must be a valid non-zero id');
    });
    it('validates non-empty user id', () => {
        expect(() => new MockDiscordContext('123', '', '789', '<@456>')).toThrow('userId must be a valid non-zero id');
    });
    it('validates non-empty channel id', () => {
        expect(() => new MockDiscordContext('123', '456', '', '<@456>')).toThrow('channelId must be a valid non-zero id');
    });
    it('validates non-empty user mention', () => {
        expect(() => new MockDiscordContext('123', '456', '789', '')).toThrow('userMention must not be empty');
    });
...
//# sourceMappingURL=mock-context.test.js.map