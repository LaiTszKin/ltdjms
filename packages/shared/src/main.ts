...
import { initializeContainer, container, TOKENS } from './infra/di/index.js';
...
async function initializeAllModules(
  pool: unknown,
  eventPublisher: { register: (handler: (event: unknown) => void) => void },
  client: Client,
  config: { getDiscordBotToken: () => string },
  logger: ReturnType<typeof createRootLogger>,
): Promise<void> {
  // ---- 1. Economy ----
  const { configureEconomyContainer, ECONOMY_TOKENS: econTokens } =
    await import('@ltdjms/economy');
  configureEconomyContainer();
  logger.info('Economy module initialized');

  // ---- 2. Dispatch ----
  const { configureDispatchContainer, DISPATCH_TOKENS: dispTokens } =
    await import('@ltdjms/dispatch');
  ...
  const balanceAdjustmentService = container.resolve(
    econTokens.BalanceAdjustmentService,
  );
  const currencyTransactionService = container.resolve(
    econTokens.CurrencyTransactionService,
  );
  const productRewardService = container.resolve(econTokens.GameRewardService);
  const escortHandoffService = container.resolve(
    dispTokens.EscortDispatchHandoffService,
  );

  // ---- 3. Shop ----
  const {
    configureContainer: configureShopContainer,
  ...
  // ---- 5. Admin ----
  const { configureAdminContainer, ADMIN_TOKENS, SlashCommandListener, SlashCommandRegistrar } =
    await import('@ltdjms/admin');
  configureAdminContainer();

  const slashCommandListener = container.resolve<{
    onInteraction: (
      interaction: {
