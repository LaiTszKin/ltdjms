...
describe('GameRewardService', () => {
    describe('creditReward', () => {
        it('should return current balance when reward is 0', async () => {
            ...
            const service = new GameRewardService(mockAccountRepo, mockTxService, mockEventPublisher);
            const balance = await service.creditReward(1, 1, 0, 'DICE_GAME_1_WIN');
        ...
        it('should throw on negative reward', async () => {
            ...
            await expect(service.creditReward(1, 1, -100, 'DICE_GAME_1_WIN')).rejects.toThrow('Reward amount cannot be negative');
        });
        it('should apply reward in a single adjustment for normal amounts', async () => {
            ...
            const service = new GameRewardService(mockAccountRepo, mockTxService, mockEventPublisher);
            const balance = await service.creditReward(1, 1, 2500, 'DICE_GAME_1_WIN');
        ...
        it('should split large rewards into multiple adjustments', async () => {
            ...
            const service = new GameRewardService(mockAccountRepo, mockTxService, mockEventPublisher);
            const balance = await service.creditReward(1, 1, 500, 'DICE_GAME_1_WIN');
            expect(balance).toBe(500);
...
//# sourceMappingURL=game-reward-service.test.js.map