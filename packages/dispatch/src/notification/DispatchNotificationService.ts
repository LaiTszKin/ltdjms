...
export class DispatchNotificationService {
  ...
  async notifyEscortOrderCreated(order: EscortDispatchOrder): Promise<void> {
    await this.sendDM(
      ...
      `📋 您有新的護航訂單 #${order.orderNumber} 待確認，請前往面板查看詳情。`,
    );
  }

  /** DM 給護航者：已被指派新訂單。 */
  async notifyEscortAssigned(order: EscortDispatchOrder): Promise<void> {
    await this.sendDM(
      String(order.escortUserId),
      `📌 您已被指派護航訂單 #${order.orderNumber}，請前往面板確認接單。`,
    );
  }

  /** DM 給管理員與客戶：護航者已確認接單。 */
  async notifyEscortConfirmed(order: EscortDispatchOrder): Promise<void> {
  ...
  /** DM 給客戶：要求確認完成。 */
  async notifyCompletionRequested(order: EscortDispatchOrder): Promise<void> {
    await this.sendDM(
      String(order.customerUserId),
      `🔔 護航者已完成服務，訂單 #${order.orderNumber} 請前往面板確認完成。若 24 小時內未確認，系統將自動完成。`,
    );
  }

  /** DM 給護航者與管理員：客戶已確認完成。 */
  async notifyCustomerConfirmed(order: EscortDispatchOrder): Promise<void> {
  ...
  /** DM 給客戶：售後案件已被接手。 */
  async notifyAfterSalesClaimed(order: EscortDispatchOrder): Promise<void> {
    await this.sendDM(
      String(order.customerUserId),
      `🛠️ 您的售後案件 #${order.orderNumber} 已被售後人員接手處理。`,
    );
  }

  /** DM 給客戶：售後案件已結案。 */
  async notifyAfterSalesClosed(order: EscortDispatchOrder): Promise<void> {
    await this.sendDM(
      String(order.customerUserId),
      `✅ 您的售後案件 #${order.orderNumber} 已結案。若有其他問題請隨時聯繫我們。`,
    );
  }
  ...
  private async sendDM(userId: string, content: string): Promise<void> {
    try {
      const client = this.gateway.requireReadyClient() as {
        users: {
          fetch(id: string): Promise<{ send(options: { content: string }): Promise<unknown> } | null>;
        };
      };
   ...
   */
  private async sendDMToStaffGuild(guildId: string, content: string): Promise<void> {
    try {
      const guild = this.gateway.findGuild(guildId) as {
        systemChannel?: { send(options: { content: string }): Promise<unknown> } | null;
      } | null;
