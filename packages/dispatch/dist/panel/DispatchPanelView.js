import { ButtonStyle } from '@ltdjms/shared';
// ============================================================
// Custom ID Constants
// ============================================================
...
// ============================================================
/** 主模式選擇面板。 */
export function buildModeSelectEmbed() {
    return {
        title: '護航派單系統',
...
/** 建立模式面板。 */
export function buildCreateModeEmbed() {
    return {
        title: '建立派單',
...
/** 指派模式面板。 */
export function buildAssignModeEmbed() {
    return {
        title: '指派訂單',
...
/** 無待派發訂單的提示。 */
export function buildNoPendingOrdersEmbed() {
    return {
        title: '無待派發訂單',
...
/** 查詢訂單失敗的錯誤訊息。 */
export function buildQueryFailedEmbed() {
    return {
        title: '查詢失敗',
...
// ============================================================
/** 模式選擇的操作列。 */
export function buildModeSelectActionRow() {
    return [buildCreateModeButton(), buildAssignModeButton(), buildViewOrdersButton()];
}
/** 訂單詳情頁的操作列。 */
export function buildOrderDetailActionRow(canConfirm, canComplete, canRequestAfterSales) {
    const buttons = [];
    if (canConfirm) {
 ...
 */
export function embedViewToApiEmbed(view) {
    const embed = {};
    if (view.title)
        embed.title = view.title;
    if (view.description)
        embed.description = view.description;
    if (view.color)
        embed.color = view.color;
 ...
 */
export function buttonsToComponents(buttons) {
    return [
        {
            type: 1, // ActionRow
...
//# sourceMappingURL=DispatchPanelView.js.map