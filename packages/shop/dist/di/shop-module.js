...
export function configureContainer(options) {
    ...
    const log = options.logger ?? container.resolve('Logger');
    // ---- Repositories ----
    ...
    container.registerInstance(SHOP_TOKENS.FiatOrderRepository, fiatOrderRepo);
    container.registerInstance(SHOP_TOKENS.RedemptionCodeRepository, redemptionCodeRepo);
    ...
    const reconciliationService = new FiatPaymentReconciliationService(fiatOrderRepo, ecpayTradeQuery, log);
    ...
    // ---- Currency Purchase ----
    const currencyPurchase = new CurrencyPurchaseService(options.productRepository, options.balanceService, options.balanceAdjustmentService, options.currencyTransactionService, options.productRewardService, log);
