import { formatFiatOrderDMMessage } from '../services/fiat-order.service.js';
import { formatPurchaseSuccessMessage } from '../services/currency-purchase.service.js';
import { ModalBuilder, TextInputBuilder, ActionRowBuilder, TextInputStyle } from 'discord.js';
import { buildShopEmbed, buildEmptyShopEmbed, buildShopComponents, buildSearchComponents, buildSearchResultEmbed, buildPaymentMethodChoiceEmbed, decodeKeyword, BUTTON_PREV_PAGE, BUTTON_NEXT_PAGE, BUTTON_BUY, BUTTON_SEARCH, BUTTON_PAY_WITH_CURRENCY, BUTTON_PAY_WITH_FIAT, BUTTON_BACK_TO_SHOP, BUTTON_SEARCH_PREV, BUTTON_SEARCH_NEXT, MODAL_SEARCH, } from '../services/shop-view.js';
/**
...
export class ShopCommandHandler {
    ...
    commandName = 'shop';
    constructor(shopService, fiatOrderService, currencyPurchaseService) {
        this.shopService = shopService;
        this.fiatOrderService = fiatOrderService;
        this.currencyPurchaseService = currencyPurchaseService;
    }
    /**
     ...
     */
    async handleInteraction(interaction, _context, customId) {
        const guildId = this.parseGuildId(interaction.getGuildId());
        if (guildId == null)
            return;
        ...
        if (customId === MODAL_SEARCH) {
            const raw = interaction.getHook();
            const keyword = raw.fields?.getTextInputValue('shop_search_input') ?? '';
     ...
     */
    async showPage(interaction, guildId, pageNum) {
     ...
     */
    async showBuySelection(interaction, guildId) {
        const page = await this.shopService.getShopPage(guildId, 1);
        if (ShopCommandHandler.pageIsEmpty(page)) {
     ...
     */
    async showCurrencyPurchaseConfirm(interaction, guildId, productId) {
     ...
     */
    async editWithComponents(interaction, embed, components) {
     ...
     */
    parseSearchCustomId(customId, prefix) {
...
//# sourceMappingURL=shop-handler.js.map