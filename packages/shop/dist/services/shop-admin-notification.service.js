...
export class ShopAdminNotificationService {
     ...
     */
    notifyAdminsOrderCreated(guildId, buyerUserId, dispatchOrder) {
        if (!dispatchOrder)
            return;
     ...
     */
    notifyAdminsProductOrderCreated(guildId, buyerUserId, product, orderType, orderReference) {
        if (!product)
            return;
        const message = this.buildAdminOrderNotification(guildId, buyerUserId, product, orderType, orderReference);
        this.notifyGuildAdmins(guildId, message);
    }
    notifyAdminsEscortOrderCreated(guildId, buyerUserId, dispatchOrder) {
        if (!dispatchOrder)
            return;
        const message = this.buildAdminEscortNotification(guildId, buyerUserId, dispatchOrder);
        this.notifyGuildAdmins(guildId, message);
    }
    notifyGuildAdmins(guildId, message) {
        ...
        try {
            ...
            if (ownerId &&
                ...
                guild.members.fetch(ownerId).then((ownerMember) => {
                    if (ownerMember && ownerMember.user) {
                        const ownerUserId = ownerMember.user.id;
                        if (selfUserId && ownerUserId === selfUserId)
                            return;
                        this.sendAdminNotification(ownerMember.user, message);
                    }
                }, (err) => this.log.debug({ guildId, ownerId, error: err }, 'Failed to retrieve guild owner for order notification'));
            }
        }
        catch (e) {
    ...
    sendAdminNotification(user, message) {
        try {
            user.send(message).catch((err) => {
                this.log.warn({ adminUserId: user.id, error: err }, 'Failed to send admin DM for order notification');
            });
        }
        catch (e) {
            this.log.warn({ adminUserId: user.id, error: e }, 'Failed to open admin DM for order notification');
        }
    }
    buildAdminOrderNotification(guildId, buyerUserId, product, orderType, orderReference) {
        const lines = [];
    ...
    buildAdminEscortNotification(guildId, buyerUserId, order) {
        const lines = [];
...
//# sourceMappingURL=shop-admin-notification.service.js.map