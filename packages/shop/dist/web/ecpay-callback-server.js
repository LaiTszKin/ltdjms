...
export class EcpayCallbackHttpServer {
    ...
    server = null;
    started = false;
    constructor(config, callbackService, logger) {
        this.config = config;
    ...
    start() {
        ...
        if (!returnUrl || returnUrl.trim().length === 0) {
            this.log.info('Skip starting ECPay callback server because no effective return URL is configured (set APP_PUBLIC_BASE_URL or ECPAY_RETURN_URL)');
            return;
        }
        ...
        if (this.config.getEcpayStageMode() && this.isPubliclyExposedBindHost(bindHost)) {
            throw new Error('ECPAY_STAGE_MODE=true 時，callback server 不可綁定公開位址。請改用 127.0.0.1 / localhost / ::1，或切換正式環境設定。');
        }
        this.app = express();
        ...
        this.app.post(callbackPath, async (req, res) => {
            try {
                ...
                const result = await this.callbackService.handleCallbackAsync(requestBody, contentType);
        ...
        this.server = this.app.listen(bindPort, bindHost, () => {
            this.log.info({ host: bindHost, port: bindPort, callbackPath }, 'ECPay callback server started');
        });
        this.started = true;
    }
    stop() {
    ...
    isPubliclyExposedBindHost(bindHost) {
...
//# sourceMappingURL=ecpay-callback-server.js.map