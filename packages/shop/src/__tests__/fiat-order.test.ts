...
describe('FiatOrder', () => {
  ...
  it('should fail validation with blank productName', () => {
    expect(() =>
      createPending(1, 2, 3, '', null, null, false, null, 'ORD001', 'PAY001', 100, new Date()),
    ).toThrow();
  });

  it('should fail validation with empty orderNumber', () => {
    expect(() =>
      createPending(1, 2, 3, 'Product', null, null, false, null, '', 'PAY001', 100, new Date()),
    ).toThrow();
  });

  it('should fail validation with non-positive amount', () => {
    expect(() =>
      createPending(1, 2, 3, 'Product', null, null, false, null, 'ORD001', 'PAY001', 0, new Date()),
    ).toThrow();
  });
  ...
  describe('helper functions', () => {
    it('isPaid should detect PAID status', () => {
      const paidOrder = FiatOrderSchema.parse(
        makeBaseOrder({ status: FiatOrderStatus.PAID, paidAt: new Date() }),
      );
    ...
    it('isExpired should detect EXPIRED status', () => {
      const expiredOrder = FiatOrderSchema.parse(
        makeBaseOrder({
    ...
    it('hasFulfillmentReward should check reward type and amount', () => {
      const orderWithReward = FiatOrderSchema.parse(
        makeBaseOrder({
